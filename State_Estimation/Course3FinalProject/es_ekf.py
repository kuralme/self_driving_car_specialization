...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_est[k] = p_est[k-1] + delta_t*v_est[k-1] + (delta_t**2 / 2)*(rotation_matrix @ imu_f.data[k-1] + g)
    v_est[k] = v_est[k-1] + delta_t*(rotation_matrix @ imu_f.data[k-1] + g)
    q_est[k] = Quaternion(axis_angle=imu_w.data[k-1] * delta_t).quat_mult_right(q_est[k-1])

    # 2. Propagate uncertainty
    F = np.identity(9)
    F[:3, 3:6] = delta_t * np.identity(3)
    F[3:6, 6:] = -(rotation_matrix @ skew_symmetric(imu_f.data[k-1].reshape((3,1))))
